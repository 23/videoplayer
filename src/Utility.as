...
public function reportPlayTime(time:Number):void {
	...
	var url:String = 'http://' + props.get('domain') + '/actions/report-play-time?uuid=' + encodeURIComponent(uuid) + '&id=' + encodeURIComponent(new String(id)) + '&time=' + encodeURIComponent(new String(time+activeElement.getNumber('start'))) + '&start=' + encodeURIComponent(activeElement.getString('start'));
